...
import { SystemConstant } from 'src/app/core/common/system.constant.js';
...
export class UserComponent implements OnInit {
  ...
  @ViewChild('modalAddEdit',{static:false}) addEditModal: ModalDirective;
  ...
  saveChange(form:NgForm) {
    if (form.valid) {
      ...
      if(fi.files.length>0){
        ...
        .then((imageUrl:any)=>{
            this.entity.Avatar=imageUrl.path;
        }).then(()=>{
          this.saveData(form);
        });
      }
